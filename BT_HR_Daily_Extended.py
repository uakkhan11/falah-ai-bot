...
def add_indicators(df):
    ...
    try:
        ...
        df['supertrend_dir'] = (df['supertrend'] > 50).astype(int)
    except:
...
def combine_signals(df):
    ...
    df['entry_signal'] = 0
    df['entry_type'] = ''
    df.loc[(df['breakout_signal'] == 1) & chand_or_st & regime_breakout, ['entry_signal', 'entry_type']] = [1, 'Breakout']
    df.loc[(df['bb_breakout_signal'] == 1) & chand_or_st & regime_breakout & (df['entry_signal'] == 0), ['entry_signal', 'entry_type']] = [1, 'BB_Breakout']
    df.loc[(df['bb_pullback_signal'] == 1) & chand_or_st & regime_default & (df['entry_signal'] == 0), ['entry_signal', 'entry_type']] = [1, 'BB_Pullback']
    return df
...
def ml_trade_filter_rolling_cv(m15_df, hourly_df):
    ...
    for train_idx, test_idx in tscv.split(X):
        ...
        model = XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=42,
                              max_depth=5, learning_rate=0.1, subsample=0.8, colsample_bytree=0.8, n_estimators=100)
...
def summarize_strategy_performance(df):
    ...
    for strat in strategies:
        ...
        summary.append({
            ...
            'Profitable Symbols': strat_df[strat_df['Total PnL'] > 0]['Symbol'].nunique(),
            'Losing Symbols': strat_df[strat_df['Total PnL'] <= 0]['Symbol'].nunique(),
            'Average Win Rate %': round(strat_df['Win Rate %'].mean(), 2),
            'Total PnL': round(strat_df['Total PnL'].sum(), 2)
        })

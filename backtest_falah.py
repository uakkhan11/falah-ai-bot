...
#################
class Backtest2025:
    def __init__(self, daily_df, hourly_df, m15_df, ml_model, ml_proba, ml_index, init_cap=100_000):
    ...
    def run(self):
        ...
        for i in range(1, len(self.m15_df)):
            ...
            # Only take trades if ML probability for profitable trade is high (above threshold)
            trade_ml_prob = self.ml_proba.get(curr_idx, 0)
            if self.position == 0:
                # Basic triple-timeframe confirmation
                if (prev['ema8'] <= prev['ema20'] and curr['ema8'] > curr['ema20']
                        and curr['adx'] > 20
                        and trade_ml_prob > 0.7):
                    qty = int(self.cash / curr['close'])
                    if qty > 0:
                        self._enter_trade(curr, qty)
            elif self.position > 0:
                self._manage_trade(curr)
        if self.position > 0:  # Close at end
    ...
    def _manage_trade(self, curr):
        ...
        # live-style: always use most conservative stop
...
if __name__ == "__main__":
    ...
    for symbol in symbols:
        ...
        bt = Backtest2025(daily, hourly, m15, ml_model, ml_proba, ml_index)
        trades = bt.run()
    ...
    print(df)
    print("\nSummary written to 2025_detailed_report.txt and 2025_backtest_summary.csv")

...
class FixedSmartHalalDataFetcher:
    ...
    def _load_instruments(self):
        ...
        print(f"✅ Loaded {len(self.token_map)} NSE equity instruments")

    def _load_halal_symbols(self):
        halal_list = get_halal_list("1ccAxmGmqHoSAj9vFiZIGuV2wM6KIfnRdSebfgx1Cy_c")
        self.halal_symbols = [s for s in halal_list if s in self.token_map]
        self.execution_stats['total_symbols'] = len(self.halal_symbols)
        print(f"✅ Halal equity symbols: {len(self.halal_symbols)}")
    ...
    def _fetch_symbol_timeframe(self, symbol, token, tf_name, config):
        to_dt = datetime.now()
        from_dt = to_dt - timedelta(days=config['lookback_days'])
        time.sleep(RATE_LIMIT_DELAY)
        candles = self.kite.historical_data(token, from_dt, to_dt, config['interval'])
        ...
        # Save file with proper columns and date first
        ...
        df_out = df[save_cols]
        df_out.to_csv(os.path.join(config['output_dir'], f"{symbol}.csv"), index=False)
        return True
        ...
        for symbol in self.halal_symbols:
            ...
            for tf_name, cfg in SMART_TIMEFRAMES.items():
                try:
                    ...
                    print(f"{symbol}-{tf_name} failed: {e}")
            if ok_count: self.execution_stats['successful_fetches'] += 1
            else:        self.execution_stats['failed_fetches'] += 1
        self._print_summary()

...
# ==== STATS ====
indicator_pass_count = {k: 0 for k in ["RSI", "EMA", "Supertrend", "AI_Score"]}
exit_reason_count = {}
...
def apply_ai_score(df):
    """Apply AI model predictions to DataFrame."""
    X = df[FEATURES].copy()
...
def track_exit_reason(reason):
    """Count how many times each exit reason was triggered."""
        ...
        for i in range(len(df) - 1):
            ...
            if rsi_pass and ema_pass and st_pass and ai_pass:
                ...
                for j in range(i + 1, len(df)):
                    ...
                    reason = None
                    trailing_sl = max(ltp - 1.5 * atr, df["low"].iloc[j - 7:j].min())

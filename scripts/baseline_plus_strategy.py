...
def wpr_signal(df):
    ...
    df['wpr_conf'] = (df['wpr'] < -80)
    return df

def combine_signals(df):
    # Regime, Chandelier, SuperTrend confirm
    regime = (df['close'] > df['ema200']) & (df['adx'] > ADX_THRESHOLD) & (df['volume'] > VOLUME_MULT * df['vol_sma20'])
    chand = df['close'] > df['chandelier_exit']
    st_up = df['supertrend_dir']==1
    # Priority: Breakout / BB_Breakout / BB_Pullback, all with CE+ST+regime. W%R only as confirm
    df['entry_signal'] = 0
    df['entry_type'] = ''
    # Loosened: allow true breakouts
    mask = regime & chand & st_up
    df.loc[(df['breakout_signal']==1) & mask & (df['wpr_conf']), ['entry_signal','entry_type']] = [1,'Breakout']
    df.loc[(df['bb_breakout_signal']==1) & mask & (df['wpr_conf']) & (df['entry_signal']==0), ['entry_signal','entry_type']] = [1,'BB_Breakout']
    df.loc[(df['bb_pullback_signal']==1) & mask & (df['wpr_conf']) & (df['entry_signal']==0), ['entry_signal','entry_type']] = [1,'BB_Pullback']
    return df
...
def backtest(df, symbol):
    ...
    for i in range(1,len(df)):
        ...
        # Higher-timeframe regime for exit (not just any signal flipping)
        regime_ok = (row['close'] > row['ema200']) and (row['adx'] > ADX_THRESHOLD)
        ...
        for pid,pos in positions.items():
            ...
            # Exit on confirmed trend reversal (multi-bar regime, not every flip)
            elif not (regime_ok and st_up): reason = 'Regime Exit'
            else: reason = None
            if reason:
...
if __name__ == "__main__":
    ...
    log_df.to_csv("baseline_plus_log.csv", index=False)

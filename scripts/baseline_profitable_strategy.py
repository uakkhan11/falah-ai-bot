...
DATA_DIR = os.path.join(BASE_DIR, "swing_data")  # Daily timeframe
...
def add_indicators(df):
    ...
    # Clean NoneType issues
    cols = ['close','donchian_high','ema200','adx','vol_sma20','bb_upper','bb_lower','wpr','atr','chandelier_exit']
...
def combine_signals(df):
    regime_filter = (df['close'] > df['ema200']) & (df['adx'] > ADX_THRESHOLD) & (df['volume'] > VOLUME_MULT * df['vol_sma20'])
    chand_confirm = df['close'] > df['chandelier_exit']
    mask = regime_filter & chand_confirm
    df['entry_signal'] = 0
    df['entry_type'] = ''
    ...
    df.loc[(df['wpr_buy_signal']==1) & mask & (df['entry_signal']==0), ['entry_signal','entry_type']] = [1,'W%R_Combo']
    return df
...
if __name__ == "__main__":
    ...
    log_df.to_csv("baseline_trade_log.csv", index=False)
